...
- 2026-07-01: Phase 15 (Homepage flagship capstone) shipped — HOMEVIS-01 closed. Post-Phase-13 the homepage handoff already rendered Console archetypes, so the capstone reduced to the hero-finale swap: the `/product/dashboard-dark.png` raster inside the held `FramedDashboard` finale in `HomepageHero.tsx` was replaced with `<Console bare data={handoffPlacementConsole} />` (reuses the Phase 13 placement payload; no new content/claims). The firewall (`data-hero-framed-dashboard` wrapper + `"DebtNext · Executive Portfolio Overview"` chrome title) is byte-identical; `HeroCinematicController.tsx` animates the wrapper's opacity only and never queries the child, so no controller change was needed. `public/product/dashboard-dark.png` retired as a standalone commit (clean rollback); `grep -rn "dashboard-dark" src/ scripts/` is zero (only the new regression spec references it, to assert absence). New structural spec `tests/responsive/hero-finale-console.spec.ts` asserts the finale holds one Console (`role="img"`) and no `<img>` raster. Local gates green: `tsc --noEmit`, `eslint src tests`, `next build`, and `scripts/check-route-js-budget.sh` (the `/` First-Load-JS total stayed under the 865,308-byte ceiling). The **LHCI Case C re-baseline number gets recorded from the PR's green CI run** (the hard merge gate; not measured locally — `next dev`/`next start` hang in this sandbox). Desktop cinematic parity stays human-verify on the Vercel preview. Branch `phase-15-homepage-capstone`.
