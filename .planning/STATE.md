...
- **Resume from:** open Phase 5.3 with one tightly-scoped plan, OR negotiate the LHCI gate. Per `docs/m5-phase-5-lhci-run.md` "Recommended next move": (1) dynamic-import GSAP + ScrollTrigger behind the mobile gate (smallest change, biggest expected simulator-LCP win); (2) conditional General Sans preload; (3) switch LHCI throttling from `simulate` to `devtools` (real Chrome throttling, closer to RUM); (4) relax the gate from 2,300 to the 2,500 ms CLAUDE.md §12 spec floor. Phase 8 (Motion) stays blocked on Phase 5 close.
